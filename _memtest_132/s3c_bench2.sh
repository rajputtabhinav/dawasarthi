#!/bin/bash
# Wait for burn end, dump logs, corrected DRAM benches, post-test health (22-DIMM config)
cd ~
...
echo "=== DMESG NEW ERRORS (tail) ==="
echo netweb | sudo -S -p '' dmesg -T 2>/dev/null | grep -iE 'error|mce|machine check|oom|out of memory|corrected' | tail -8
echo "=== SEL POST ==="
